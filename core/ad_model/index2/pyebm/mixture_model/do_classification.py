...
def Reject(data_AD,data_CN):
    ...
    for i in range(m[1]):
        for j in range(m[2]):
            Dcni=np.reshape(data_CN[:,i,j],[-1,1]);#获取CN病人的七条生物标志物的均值和方差
            cn_mean=np.nanmean(Dcni);
            cn_std = np.nanstd(Dcni)+0.001;
            params_raw[i,0,j]=cn_mean; params_raw[i,1,j]=cn_std;
                      
            Dadi=np.reshape(data_AD[:,i,j],[-1,1]);#获取AD病人的七条生物标志物的均值和方差
        ...
        idx_in_cn=np.where(py<=0.500000)
        ...
        idx_in_ad=np.where(py>0.500000)
...
#得到的是容易确认的AD，CN数据，以及七种生物标志物对其的均值和标准差
#这里通过置信度0.5确认是否
