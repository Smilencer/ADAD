...
def GMM_Control(Data_all,Ncni,Nadi,params_nobias,type_opt=1,itvl=1.96,params_pruned=[]):
    ...
    for i in range(Nevents):
        ...
        if type_opt==1:
            bnds[-1,0]=0.01;
            bnds[-1,1]=0.99;            
            for j in range(Nfeats):#均值+-一个率*（方差/数量开根号）
...
def calculate_prob_mm(Data,params,val_invalid=0.5):
    ...
    for i in range(0,m[1]):
        ...
        norm_pre=scipy.stats.norm(loc=paramsi[0], scale=paramsi[1]);#通过均值和方差拟合一段连续的正态分布
        likeli_pre=norm_pre.pdf(Datai_valid);#得到CN患者对于当前这种病的后验概率
        norm_post=scipy.stats.norm(loc=paramsi[2],scale=paramsi[3]);
        likeli_post=norm_post.pdf(Datai_valid);#得到AD患者对于当前这种病的后验概率
