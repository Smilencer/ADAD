...
from core.ad_model.index2.pyebm import core_utilities as cu
import numpy as np
...
def fit(DataIn,MethodOptions=False,VerboseOptions=False,Factors=None,Labels = None,DataTest=[],Groups=[]):
    ...
    for i in range(len(data_AD_raw_list)): ## For each bootstrapped iteration
        ...
        #BiomarkerParams包含三种参数CN每种生物标记物的均值和方差，AD每种生物标记物的均值和方差，以及混合模型参数（都是训练得到的）
        #p_yes是每种生物标记物对病情产生影响的概率（分出AD和CN两种病症，归一化后得到）p_no=1-p_yes
        #likeli_post，likeli_pre后验概率，通过高斯模型的概率密度函数，带入每一个特征值后可以得到
        ...
        #pi0事件顺序，event_centers事件顺序对应的概率，通过拟合Mallow模型得到，利用kendll距离
