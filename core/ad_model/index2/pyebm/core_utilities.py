...
from core.ad_model.index2.pyebm.central_ordering import generalized_mallows as gm
from core.ad_model.index2.pyebm.inputs_outputs import prepare_outputs as po
from core.ad_model.index2.pyebm.inputs_outputs import visualize
from core.ad_model.index2.pyebm.inputs_outputs import prepare_inputs as pi
from core.ad_model.index2.pyebm.central_ordering import data_likelihood as dl
...
def do_mixturemodel(DMO,data_AD_raw,data_CN_raw,Data_all,Groups,GroupValues):
        ...
        if DMO.MixtureModel[:3]=='GMM':
            ...
            if DMO.MixtureModel=='GMMvv2':
                ...
                while flag_stop==0:#先训练高斯分布参数，再训练混合模型参数
...
def patient_staging(pi0,event_centers,likeli_post,likeli_pre,type_staging):
    ...
    for j in range(m[1]+1):#np.nanprod当axis=1时横向相乘,空值设为1，p_yes_perm[:,:j]列数逐渐增多，p_no_perm[:,j:]列数逐渐减小
